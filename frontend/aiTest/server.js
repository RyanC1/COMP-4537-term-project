import { APP_CONFIG } from "./strings.js";
...
sendButton.addEventListener("click", async () => {
  ...
  if(!auth) {
      const res = await fetch(APP_CONFIG.API_AUTH_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email:"john@john.com", password:"123" })
      });
  ...
  try {
    const response = await fetch(APP_CONFIG.API_AI_URL, {
      method: "POST",
      headers,
